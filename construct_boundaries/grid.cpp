...
grid::grid(octreeNode* _node, const double* _center, double _sideLength)
{
	...
	{
		center[d] = _center[d];
		minCoord[d] = center[d] - 2.5*sideLength;
		plane[d] = 0;
	}
	b = 0;
	...
	for (int i = 1; i < 4; i++)
	{
		for (int j = 1; j < 4; j++)
		{
			...
			{
				double tmpCenter[3];
				tmpCenter[0] = center[0] + (i - 2)*sideLength;
				tmpCenter[1] = center[1] + (j - 2)*sideLength;
				tmpCenter[2] = center[2] + (k - 2)*sideLength;
				gc[i][j][k].setCenter(tmpCenter);
				gc[i][j][k].setSideLength(sideLength);
			}
		}
	}
	gc[3][3][3].setWorkingNode(workingNeighbor[0]);
	gc[2][3][3].setWorkingNode(workingNeighbor[1]);
	gc[1][3][3].setWorkingNode(workingNeighbor[2]);
	gc[3][2][3].setWorkingNode(workingNeighbor[3]);
	gc[2][2][3].setWorkingNode(workingNeighbor[4]);
	gc[1][2][3].setWorkingNode(workingNeighbor[5]);
	gc[3][1][3].setWorkingNode(workingNeighbor[6]);
	gc[2][1][3].setWorkingNode(workingNeighbor[7]);
	gc[1][1][3].setWorkingNode(workingNeighbor[8]);
	gc[3][3][2].setWorkingNode(workingNeighbor[9]);
	gc[2][3][2].setWorkingNode(workingNeighbor[10]);
	gc[1][3][2].setWorkingNode(workingNeighbor[11]);
	gc[3][2][2].setWorkingNode(workingNeighbor[12]);
	gc[1][2][2].setWorkingNode(workingNeighbor[13]);
	gc[3][1][2].setWorkingNode(workingNeighbor[14]);
	gc[2][1][2].setWorkingNode(workingNeighbor[15]);
	gc[1][1][2].setWorkingNode(workingNeighbor[16]);
	gc[3][3][1].setWorkingNode(workingNeighbor[17]);
	gc[2][3][1].setWorkingNode(workingNeighbor[18]);
	gc[1][3][1].setWorkingNode(workingNeighbor[19]);
	gc[3][2][1].setWorkingNode(workingNeighbor[20]);
	gc[2][2][1].setWorkingNode(workingNeighbor[21]);
	gc[1][2][1].setWorkingNode(workingNeighbor[22]);
	gc[3][1][1].setWorkingNode(workingNeighbor[23]);
	gc[2][1][1].setWorkingNode(workingNeighbor[24]);
	gc[1][1][1].setWorkingNode(workingNeighbor[25]);
	gc[2][2][2].setWorkingNode(workingNode);
}
...
bool grid::computeNormal(const vector<double*> &v, vector<double*> &normal, double* myb)
{
		...
		for (int j = 1; j < 4; j++)
		{
			for (int k = 1; k < 4; k++)
			{
				if (gc[i][j][k].getWorkingNode() != NULL) {
					const vector<int> & nv = gc[i][j][k].getInCellVertex();
					...
					if (nvsize > 0) {
						if (nvsize > 1) {
							...
							double currDist = 0;
							Vector3d currV(v[cv[0]]);
							int maxVI = -1;
							int secondMaxVI = -1;
							for (int nvi = 0; nvi < nvsize; nvi++) {
								Vector3d neighborV(v[nv[nvi]]);
								currDist = (currV - neighborV).L2Norm();
								if (currDist > maxDist || maxVI < 0) {
									if (maxVI >= 0 && (maxDist >= secondMaxDist || secondMaxVI < 0)) {
								...
								else if (currDist >= secondMaxDist || secondMaxVI < 0) {
									secondMaxDist = currDist;
									secondMaxVI = nv[nvi];
								}
							}
							if (maxVI < 0 || secondMaxVI < 0) {
								cout << maxVI << " " << secondMaxVI << " " << nvsize << endl;
								cin.get();
							}
							tmpPoint.push_back(maxVI);
							tmpPoint.push_back(secondMaxVI);
						}
						else {
							tmpPoint.push_back(nv[0]);
						}
					}
				}
			}
	...
	for (int d = 0; d < 3; d++)
	{
		...
		{
			centroid[d] = centroid[d] + v[tmpPoint[i]][d];
		}
	...
	for (int d1 = 0; d1 < 3; d1++)
	{
		for (int d2 = 0; d2 < 3; d2++)
		{
			...
			for (int i = 0; i < nvsize; i++)
			{
				cov[d1][d2] = cov[d1][d2] + (v[tmpPoint[i]][d1] - centroid[d1])*(v[tmpPoint[i]][d2] - centroid[d2]);
			}
		}
	}
...
bool gridCell::setEdgesAndCheckIntersection(double innerLength, double halfLength, const double* plane, const double b, gridCell gc[5][5][5], int cx, int cy, int cz, const double* rMaxC, const double* rMinC)
{
	...
	for (int i = 0; i < 144; i++)
	{
		...
		if (fv1*fv2 < 0 || fabs(fv1) < 0.00000001 || fabs(fv2) < 0.00000001)
		{
			...
			for (int nc = 0; nc < edgeNeighborCellSize[i]; nc++)
			{
				...
				if (x > 3 || y > 3 || z > 3 || x < 1 || y < 1 || z < 1 || gc[x][y][z].getNumOfVertex() > 0)
				{
					allEmpty = false;
					break;
				}
